...
export const FEEDS = [
  ...
  { source: "NPR", topic: "Technology", url: "https://feeds.npr.org/1019/rss.xml" },
  ...
  { source: "The Guardian", topic: "Technology", url: "https://www.theguardian.com/technology/rss" },
];

const PER_FEED = 2; // newest N stories considered from each feed row above
const TOTAL_CAP = 10; // hard ceiling on stories shown, regardless of feed counts
...
async function fetchFreshHeadlines() {
  ...
  // Hard-cap the list to keep it short and easy to consume — the 10 newest
  // across all feeds, even if some feeds contributed more than one. Capping
  // here also means we only fetch fallback images for stories we'll show.
  const top = unique.slice(0, TOTAL_CAP);
